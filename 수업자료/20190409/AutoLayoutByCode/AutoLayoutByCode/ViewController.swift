...
//    activateVisualFormat()
  ...
  private func setupAutoresizingMask() {
    firstView.frame = CGRect(
        x: 50, y: 50, width: view.bounds.width - 100, height: view.bounds.height - 100
    )
    secondView.frame = CGRect(
        x: 40, y: 40, width: firstView.bounds.width - 80, height: firstView.bounds.height - 80
    )
  ...
  private func activateLayoutAnchors() {
    // iOS 9.0 이상
    // 코드로 오토리사이징시 false로 끄기
    firstView.translatesAutoresizingMaskIntoConstraints = false
    
    // autoLayout - view.safeAreaLayoutGuide.topAnchor
