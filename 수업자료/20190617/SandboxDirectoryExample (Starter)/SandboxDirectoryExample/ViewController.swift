...
class ViewController: UIViewController {
    ...
    @IBAction private func printBundleDirectory(_ sender: UIButton) {
        ...
        print("NSHomeDicrectory :", FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0])
    }
    ...
    @IBAction private func printDocumentsDirectory(_ sender: UIButton) {
        ...
        var documentsUrl = FileManager.default.urls(
            ...
            // ios에선 무조건 userDomainMask 사용
            in: .userDomainMask
            ).first!
        print(documentsUrl.path)
        
        let test = documentsUrl.appendingPathComponent("test").appendingPathExtension("txt")
        try? (["t": "est"] as NSDictionary).write(to: test)
//        try? "AAA".write(to: documentsUrl, atomically: true, encoding: .utf8)
...
//            }
    ...
    @IBAction private func bundleFile(_ sender: UIButton) {
        ...
        melonChart.sorted { $0.0 < $1.0 }.forEach {
            (key, value) in
            print("\n[ \(key) 멜론 Top 10 ]")
    ...
    private func saveFileUrl() -> URL {
        ...
        let fileUrl = saveDir.appendingPathComponent("SongsChart").appendingPathExtension("plist")
    ...
    @IBAction private func loadFile(_ sender: UIButton) {
        ...
        melonChart.sorted { $0.0 < $1.0 }.forEach { (key, value) in
            print("\n[ \(key) 멜론 Top 10 ]")
            value.forEach { print(">", $0) }
        }
